...
class BaseRepository(Generic[ModelType]):
    ...
    def create(self, obj_in: Any, customer_id: int) -> ModelType:
        ...
        db_obj = self.model(**obj_data, customer_id=customer_id)
        self.db.add(db_obj)
        self.db.commit()
    ...
    def get_by_customer(self, customer_id: int, skip: int = 0, limit: int = 100) -> List[ModelType]:
        return self.db.query(self.model).filter(self.model.customer_id == customer_id).offset(skip).limit(limit).all()

    def get_by_id_and_customer(self, id: int, customer_id: int) -> Optional[ModelType]:
        return self.db.query(self.model).filter(self.model.id == id, self.model.customer_id == customer_id).first()
